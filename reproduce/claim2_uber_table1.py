...
class TwoT4MU:
    ...
    def _one(self, shard, which):
        ...
        with torch.cuda.device(d):
            ...
            # alpha=beta=1: A=Y*observed and B=Yhat*observed.
            A=self.y[shard]*self.obs[shard]; B=pred*self.obs[shard]
    ...
    def heldout_mse(self):
        def one(s):
            d=self.dev[s]
            with torch.cuda.device(d):
                p=[torch.as_tensor(x,device=d) for x in self.cpu]; p[3]=p[3][self.slices[s]]
                e=(self.y[s]-torch.einsum(self.model,*p))
                z=(e[self.hold[s]].square().sum().cpu(), self.hold[s].sum().cpu()); torch.cuda.synchronize(d); return z
        z=list(self.pool.map(one,range(2))); return float(sum(x[0] for x in z)/sum(x[1] for x in z))
    def fit(self, iters):
        hist=[]; start=time.perf_counter()
        for _ in range(iters): self.step(); hist.append(self.heldout_mse())
